        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
<!--                以前的路由跳转，从a路由跳转到b路由，这里在加入购物车，在路由跳转之前，发请求，
                    把购买的产品的信息通过请求的形式通知服务器，服务器进行存储&ndash;&gt;-->
                <a href="javascript:" @click="AddOrUpdateShopCart()">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
    ...
    methods:{
      ...
      //加入购物车的回调函数
       async AddOrUpdateShopCart(){
        //派发action----产品加入数据库（通知服务器）
        //成功：服务器存储成功------路由跳转（传递参数）
        //失败：-----提示给用户
        //AddOrUpdateShopCart代码加上了async,返回promise对象，要么成功，要么失败
        try {
          ...
          //路由跳转
          //一些简单的数据skuid，通过query的形式传递给路由组件传递过去
          //产品信息的数据【比较复杂:skuinfo】的用会话存储（不持久化，会话结束数据消失）
          //本地存储|会话存储，一般存储字符串
          sessionStorage.setItem('STUINFO',JSON.stringify(this.skuInfo))
            this.$router.push({
              name:'addcartsuccess',
              query:{
                skuNum:this.skuNum
